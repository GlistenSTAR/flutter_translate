...
import 'package:tutor/view/tutor/schedule/change_class.dart';
import 'package:tutor/view/tutor/schedule/change_slot.dart';
...
class _ScheduleViewState extends State<ScheduleView> {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return ScheduleCell(
            ...
            top: () {
              if (index == 0) return;
              showChangeSlot(index - 1, true);
            },
            bottom: () {
              if (index == 15) return;
              showChangeSlot(index, true);
            });
      },
      itemCount: 16,
    );
                      ...
                      Container(
                        ...
                        child: ElevatedButton(
                          onPressed: () {
                            ...
                            showChangeClass(model);
                          },
                          child: Text("เปลี่ยนวันและเวลาสอน"),
  ...
  Future<Map<String, dynamic>> getClassInfo() async {
    QuerySnapshot snapshot = await FirebaseFirestore.instance
        .collection("ClassIDs")
        .where("tutor_id", isEqualTo: Globals.currentUser!.uid)
        .where("class_date",
    ...
    List<int> slots =
        await Util.getUnavailabeSlot(Globals.currentUser!.uid, _currentDate);
  ...
  void showChangeClass(ClassModel model) async {
    dynamic result = await Navigator.of(context).push(
      PageRouteBuilder(
          opaque: false,
          barrierColor: Colors.black54,
          pageBuilder: (_, __, ___) => ChangeClass(model: model)),
    );

    if (result != null && result is Map<String, dynamic>) {
      Map<String, dynamic> prevData = {
        "class_date": model.date,
        "class_beginTime": model.beginTime,
        "class_endTime": model.endTime,
        "class_time": model.time,
      };

      result["previous"] = prevData;

      await showDialog(
        context: context,
        builder: (context) => FutureProgressDialog(
          Util.updateClass(model.id, result),
        ),
      );
  ...
  void showChangeSlot(int slot, bool status) async {
    dynamic result = await Navigator.of(context).push(
      PageRouteBuilder(
        opaque: false,
        barrierColor: Colors.black54,
        pageBuilder: (_, __, ___) => ChangeSlot(isAvaliable: status),
      ),
    );
    ...
    if (result != null && result is bool) {
      ...
      await showDialog(
        context: context,
        builder: (context) => FutureProgressDialog(
          Util.updateTimeSlot(Globals.currentUser!.uid, _currentDate, slots),
        ),
      );
